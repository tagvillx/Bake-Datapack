...
execute if score $diff quest.item_check matches 0 store success score $exclude quest.item_check run data modify storage quest:item_check Items[0].tag merge from storage quest:item_check Item.exclude_tag
execute if score $exclude quest.item_check matches 0 run scoreboard players set $diff quest.item_check 1
execute if score $diff quest.item_check matches 0 run function quest:item_check/count
data remove storage quest:item_check Items[0]
execute if score $output quest.item_check matches 0 if data storage quest:item_check Items[] run function quest:item_check/loop
