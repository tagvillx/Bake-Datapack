...
execute as @a[scores={trident_copy_drop=1..}] at @s if entity @e[type=item,sort=nearest,distance=..4,predicate=sanctuary_ench:trident_copy/fake,limit=1] run function sanctuary_ench:trident_copy/copy_count
execute as @a[scores={trident_copy_drop=1..}] at @s as @e[type=item,sort=nearest,distance=..4,predicate=sanctuary_ench:trident_copy/fake] at @s run function sanctuary_ench:trident_copy/fake_dropped
scoreboard players reset @a trident_copy_drop
